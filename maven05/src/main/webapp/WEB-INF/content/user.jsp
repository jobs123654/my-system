...
<body>
	 ...
	 <label style="margin-left: 5%">  <a href="home">首页</a>/业务员列表</label>
	  <label style="margin-left: 80%">  <a title="添加" href="show_add_user">+</a></label>
	 </div>
     ...
     <c:if test="${page.stop>0}">
            <c:if test="${page.current!=1 }">
			    <a href="user_list?c=<c:out value="${page.current -1}"></c:out>"> [pre]</a>
			   </c:if>
         
             <c:forEach begin="${page.start}" end="${page.stop}" var="i">
			      <a href="user_list?c=<c:out value="${i }"></c:out>">
			      <c:out value="${i }"></c:out>
			     </a>
			   </c:forEach>
			   
			     <c:if test="${page.current!=page.totalPage }">
 <a href="user_list?c=<c:out value="${page.current +1}"></c:out>"> [next]</a>    
